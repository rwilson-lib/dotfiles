...
#+begin_src emacs-lisp
  (use-package company
    :init
    ;; (define-prefix-command 'company-active-map)
    ;; (define-key global-map (kbd "<tab>") 'company-active-map)

    ;; :hook (prog-mode . company-mode)
    :bind (:map company-active-map
                ("SPC t" . complete-tag)        ;; etags
                ("SPC d" . cape-dabbrev)        ;; or dabbrev-completion
                ("SPC f" . cape-file)
                ("SPC k" . cape-keyword)
                ("SPC s" . cape-symbol)
                ("SPC a" . cape-abbrev)
